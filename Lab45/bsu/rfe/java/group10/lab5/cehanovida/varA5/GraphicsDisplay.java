    ...
    private void paintGrid(Graphics2D canvas) {
	    ...
	    protected void paintMarkers(Graphics2D canvas) {
	        ...
	        for (double[] point : graphicsData) {
	            ...
	            drawTriangle(canvas, point);
	        }
	    }
	
	 // Метод для рисования обычного треугольника
	    private void drawTriangle(Graphics2D canvas, double[] point) {
	        // Центр маркера
	        ...
	        // Координаты вершин обычного треугольника
	        int[] xPoints = {
	            centerX - size, centerX + size, centerX // Левый, правый и верхний углы
	        };
	        int[] yPoints = {
	            centerY - size, centerY - size, centerY + size // Верхний угол выше, нижний угол ниже
	        };

	        // Рисуем обычный треугольник
	    ...
	    private boolean isAscending(double[] point) {
	        int wholePartY = (int) Math.abs(point[1]);
	        int sum = 0;
	        
	        while (wholePartY > 0) {
	            sum += wholePartY % 10;
	            wholePartY /= 10;       // Убираем последнюю цифру
	        }
	        
	        // Проверяем, меньше ли сумма десяти
	        return (sum < 10);
	    }
